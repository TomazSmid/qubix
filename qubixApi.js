...
var rl = readline.createInterface({
    input: fs.createReadStream('api test data.csv')
});

var bridges = {};   //this will store the active bridges while the app is running
var key;        //this will hold the property names/CSV column names
...
rl.on('line', function (line) {
    ...
    if(lineNumber == 0){    //This app assumes the first line of the CSV contains field names
        key = line.split(",");    //This app assumes the CSV is "," delimited and does not use any commas in the content
    }else{
        bridge = {};
        content = line.split(",");      //This app assumes the CSV is "," delimited and does not use any commas in the content
        for(i=0; i<content.length; i++){
            bridge[key[i]] = content[i];    //populates bridge object with key value pairs
        }
...
router.get('/bridges/type/:type', function(req, res) {
    ...
    res.json(page.bridgesOfType);   //responds with only the requested information, a link to next page is provided in headers
});
